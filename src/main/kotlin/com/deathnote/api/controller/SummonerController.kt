...
import com.deathnote.api.model.vo.SummonerInfoDto
...
class SummonerController(private val summonerService: SummonerService) {
    ...
    fun getSummoner(): ResponseEntity<Summoner> {
        return ResponseEntity.ok(summonerService.getSummonerInfo())
    }

    @GetMapping("/{summonerName}")
    fun search(@PathVariable summonerName: String): ResponseEntity<SummonerInfoDto> {
        val summonerInfoDto = SummonerInfoDto.of(summonerService.getSummonerInfo(summonerName))
